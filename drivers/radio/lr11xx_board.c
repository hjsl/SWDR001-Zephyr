...
static int lr11xx_init(const struct device *dev)
{
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_LR11XX_THREAD_STACK_SIZE,
			...
			K_PRIO_COOP(CONFIG_LR11XX_THREAD_PRIORITY), 0, K_NO_WAIT);
#endif // CONFIG_LR11XX_EVENT_TRIGGER_OWN_THREAD
...
#define LR11XX_DEVICE_INIT(inst)                                                                   \
	DEVICE_DT_INST_DEFINE(inst, lr11xx_init, PM_DEVICE_DT_INST_GET(inst), &lr11xx_data_##inst, \
			      &lr11xx_config_##inst, POST_KERNEL, CONFIG_LR11XX_INIT_PRIORITY,     \
			      NULL);

#define LR11XX_CFG_PWR_EN(inst) .pwr_en = GPIO_DT_SPEC_INST_GET(inst, pwr_en_gpios),

#define LR11XX_CFG_LNA_EN(inst) .lna_en = GPIO_DT_SPEC_INST_GET(inst, gps_lna_en_gpios),

#define LR11XX_CFG_RF_SW_EN(inst) .enable = DT_INST_PROP(inst, rf_sw_enable),

#define LR11XX_CFG_RF_STANDBY_MODE(inst) .standby = DT_INST_PROP(inst, rf_sw_standby_mode),

#define LR11XX_CFG_RF_SW_RX_MODE(inst) .rx = DT_INST_PROP(inst, rf_sw_rx_mode),

#define LR11XX_CFG_RF_SW_TX_MODE(inst) .tx = DT_INST_PROP(inst, rf_sw_tx_mode),

#define LR11XX_CFG_RF_SW_TX_HP_MODE(inst) .tx_hp = DT_INST_PROP(inst, rf_sw_tx_hp_mode),

#define LR11XX_CFG_RF_SW_TX_HF_MODE(inst) .tx_hf = DT_INST_PROP(inst, rf_sw_tx_hf_mode),

#define LR11XX_CFG_RF_SW_WIFI_MODE(inst) .wifi = DT_INST_PROP(inst, rf_sw_wifi_mode),

#define LR11XX_CFG_RF_SW_GNSS_MODE(inst) .gnss = DT_INST_PROP(inst, rf_sw_gnss_mode),
			...
			COND_CODE_1(DT_INST_NODE_HAS_PROP(inst, rf_sw_tx_mode),                    \
				...
				COND_CODE_1(DT_INST_NODE_HAS_PROP(inst, rf_sw_tx_hf_mode),         \
					...
					COND_CODE_1(DT_INST_NODE_HAS_PROP(inst, rf_sw_wifi_mode),  \
						...
						COND_CODE_1(DT_INST_NODE_HAS_PROP(                 \
								    inst, rf_sw_gnss_mode),        \
...
#define LR11XX_DEFINE(inst)                                                                        \
	...
	static const struct lr11xx_hal_context_cfg_t lr11xx_config_##inst = LR11XX_CONFIG(inst);   \
